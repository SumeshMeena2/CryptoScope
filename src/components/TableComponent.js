...
const TableComponent = () => {
  ...
  return (
    <>
    <div
        className='flex flex-col mt-9 border border-gray-100 rounded w-full h-full overflow-x-hidden '
        >
       {
        cryptoData ?    <table className='w-full table-auto max-sm:text-[8px] '>      
            <thead className='capitalize text-base text-gray-100 font-medium border-b border-gray-100'>
                <tr className='max-sm:text-[12px]' >
              <th className='py-1'>asset</th>
              <th className='py-1 max-sm:hidden'>name</th>
              <th className='py-1'>price</th>
              <th className='py-1  max-md:hidden'>total volume</th>
              <th className='py-1'>market cap change</th>
              <th className='py-1 max-lg:hidden'>1H</th>
              <th className='py-1 max-lg:hidden'>24H</th>
              <th className='py-1 max-lg:hidden'>7D</th>
             </tr>
             </thead>
            ...
            <tbody>
             ...
             {cryptoData.map((data) => {
                 ...
                 return(
                    <tr    
                    key={data.id}
                    className='max-sm:text-[12px] text-center text-base border-b border-gray-100 hover:bg-gray-200 last:border-b-0'>
                  ...
                  <SaveBtn data={data} />
                    ...
                    <img className='w-[1.8rem] h-[1.8rem] mx-1.5' src={data.image} />
                    <span> <Link to = {`/${data.id}`} className='cursor-pointer'> {data.symbol} </Link></span>
                    
                    </td>

                    
                    <td className='py-4  max-sm:hidden'> 
                       <Link to = {`/${data.id}`} className='cursor-pointer '> {data.name} </Link>
                    </td>
                    <td className='py-4'>
  {(() => {
    try {
      // Attempt to format with the given currency
      return new Intl.NumberFormat('en-IN', {
        style: 'currency',
        currency: currency,
      }).format(data.current_price);
    } catch (error) {
      console.error("Invalid currency, defaulting to USD:", error);
      // Fallback to USD if the currency is invalid
      return new Intl.NumberFormat('en-IN', {
        style: 'currency',
        currency: 'USD',
      }).format(data.current_price);
                    ...
                    <td className='py-4  max-md:hidden'>{data.total_volume}</td>
                    <td className='py-4'>{data.market_cap_change_percentage_24h}</td>

                    <td className =  {  data.price_change_percentage_1h_in_currency > 0 ? 'text-green  py-4  max-lg:hidden '  : 'text-red py-4 max-lg:hidden'}>{Number(data.price_change_percentage_1h_in_currency).toFixed(4)}</td>

                    <td className={data.price_change_percentage_24h_in_currency > 0 ? 'text-green  py-4   max-lg:hidden '  : 'text-red py-4  max-lg:hidden'}>{Number(data.price_change_percentage_24h_in_currency).toFixed(4)}</td>

                    <td className={data.price_change_percentage_7d_in_currency > 0 ? 'text-green  py-4   max-lg:hidden'  : 'text-red py-4  max-lg:hidden'}>{Number(data.price_change_percentage_7d_in_currency).toFixed(4)}</td>
     ...
     <div className='flex items-center justify-between mt-4 capitalize h-[2-rem] max-sm:flex-col-reverse '>
        <span> Data Provided by <a className='text-cyan' href='https://coingecko.com'>CoinGecko</a> </span>
       <Pagination/>
     </div>
     </>
  )
